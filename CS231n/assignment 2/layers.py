...
def affine_backward(dout, cache):
    ...
    x, w, b = cache
    t = np.copy(x)
    t = t.reshape(t.shape[0],-1) # 中间变量
    
    db = np.sum(dout, axis=0) # 列和
    dx = np.dot(dout,w.T).reshape(np.shape(x))
    dw = np.dot(t.T, dout)
...
def batchnorm_backward(dout, cache):
    ...
    m = x.shape[0]
    dx_hat = gamma * dout
    d_mean = -np.sum(dx_hat / np.sqrt(sample_var + eps),axis=0)
    d_var = np.sum(dx_hat * (x - sample_mean) * -0.5 * np.power(sample_var + eps,-1.5),axis=0)
    dx = dx_hat / np.sqrt(sample_var + eps) + 2 * d_var * (x - sample_mean) / m + d_mean / m
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    x_hat = (x - sample_mean) / np.sqrt(sample.var + eps) # 这里 broadcasting 机制不能工作
    out = gamma * x_hat + beta
    cache = (x,x_hat,gamma,beta,sample_mean,sample_var)
    '''
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for num in range(N):
        for _filter in range(F):
            for height in range(H_new):
                for width in range(W_new):
                    start_h = stride * height
                    start_w = stride * width
                    neural_prod = np.sum(x_pad[num, :, start_h:start_h+HH, start_w:start_w+WW] * w[_filter, :, :, :])
                    out[num,_filter,height, width] = neural_prod + b[_filter]   # 切记加偏置
        ...
        for _filter in range(F):
            for height in range(H_new):
                for width in range(W_new):
                    ...
                    start_w = stride * width
                    # 这里的计算需要想一想
                    dx_pad[num, :, start_h:start_h+HH, start_w:start_w+WW] += w[_filter,:,:,:] * dout[num,_filter,height,width]
                    dw[_filter,:,:,:] +=  x_pad[num, :, start_h:start_h+HH, start_w:start_w+WW] * dout[num,_filter,height,width]
                    db[_filter] += dout[num,_filter,height,width]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for height in range(H_new):
        for width in range(W_new):
            start_h = stride * height
            start_w = stride * width
            out[:,:,height,width] = np.max(x[:,:,start_h:start_h + pool_height, start_w:start_w + pool_width],axis=(2,3))
