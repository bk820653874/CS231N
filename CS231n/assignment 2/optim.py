...
def rmsprop(w, dw, config=None):
    ...
    config.setdefault('decay_rate', 0.99) # 衰减率
    config.setdefault('epsilon', 1e-8) # 防止除零的参数
    config.setdefault('cache', np.zeros_like(w)) # 梯度积累向量
    ...
    next_w = w - config['learning_rate'] / (config['epsilon'] + np.sqrt(config['cache'])) * dw
...
def adagrad(w, dw, config=None):
    """
    Used the AdaGrad update rule(my vision.)
